...
export const command =
  ...
  `    set a to (artist of every track of library playlist 1) as text\n` +
  `    set al to (album of every track of library playlist 1) as text\n` +
  `    set p to (played count of every track of library playlist 1) as text\n` +
  `  end tell\n` +
...
const parse = (output) => {
  ...
  const items = (output || "").split(RS).map((slot) => {
    const [cover, url] = slot.split(FS);
    return { cover: (cover || "").trim(), url: (url || "").trim() };
  }).filter((it) => {
    if (!it.cover || seen.has(it.cover)) return false;
    seen.add(it.cover);
    return true;
  });
